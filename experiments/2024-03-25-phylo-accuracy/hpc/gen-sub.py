...
def main():
    ...
    for condition_dict in combo_list:
        ...
        if hpc_account == "NONE":
            file_str = file_str.replace("<<ACCOUNT_SBATCH_ARG>>", "")
        else:
            account_sbatch_arg = f"#SBATCH --account={hpc_account}"
            file_str = file_str.replace("<<ACCOUNT_SBATCH_ARG>>", account_sbatch_arg)
